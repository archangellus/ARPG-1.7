...
namespace PLAYERTWO.ARPGProject
{
    ...
    public class UICharacterSelection : MonoBehaviour
    {
        ...
        [Tooltip(
            "The confirmation message shown before deleting a Character. Use '{0}' as a "
                + "placeholder for the Character's name."
        )]
        [TextArea]
        public string deleteCharacterConfirmationMessage =
        ...
        protected virtual void OnDeleteCharacterClicked()
        {
            if (m_currentCharacterId < 0 || m_currentCharacterId >= m_characters.Count)
                return;

            var character = m_characters[m_currentCharacterId].character;
            var message = string.Format(deleteCharacterConfirmationMessage, character.name);
            UIConfirmationScreen.instance.Show(message, ConfirmDeleteCharacter);
        }

        protected virtual void ConfirmDeleteCharacter()
        {
