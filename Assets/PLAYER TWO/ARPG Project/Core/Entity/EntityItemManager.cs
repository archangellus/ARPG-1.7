...
namespace PLAYERTWO.ARPGProject
{
    ...
    public partial class EntityItemManager : MonoBehaviour
    {
        ...
        /// <summary>
        /// Consumes an item from the consumable items list and applies its effects. The item is
        /// only removed from the list once its Consume call reports success.
        /// </summary>
        ...
        public virtual void ConsumeItem(int index)
        {
            ...
            var consumable = item.GetConsumable();

            if (item.stack > 0 && consumable.CanConsume(entity))
                consumable.Consume(entity, item, () => RemoveConsumableStack(item));
        }

        /// <summary>
        /// Decrements the stack of the given consumable Item Instance and clears its slot if it
        /// reaches zero. Used to finalize consumption for consumables that only complete
        /// asynchronously (e.g. a Town Portal Scroll).
        /// </summary>
        /// <param name="instance">The Item Instance to remove one unit from.</param>
        public virtual void RemoveConsumableStack(ItemInstance instance)
        {
            var index = m_consumables.FindIndex(c => c == instance);

            if (index < 0 || instance.stack <= 0)
                return;
