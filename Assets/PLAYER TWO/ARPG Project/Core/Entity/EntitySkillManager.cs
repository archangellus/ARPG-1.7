...
namespace PLAYERTWO.ARPGProject
{
    ...
    public class EntitySkillManager : MonoBehaviour
    {
        ...
        protected Dictionary<SkillInstance, Vector3> m_castMousePositions = new();
            ...
            switch (performingInstance.data.castingOrigin)
            {
                ...
                case Skill.CastingOrigin.TargetCenter:
                    ...
                    if (m_entity.target)
                    {
                        ...
                        position = GetCachedMousePosition();
                    break;
                case Skill.CastingOrigin.TargetFloor:
                    ...
                    if (m_entity.target)
                    {
                        ...
                        position = GetFloorPoint(GetCachedMousePosition());
                    break;
            }
        ...
        /// <summary>
        /// Returns the cached mouse position for the current cast, or the current mouse position if it hasn't been cached yet.
        /// </summary>
        /// <returns>The cached mouse position for the current cast, or the current mouse position if it hasn't been cached yet.</returns>
        protected virtual Vector3 GetCachedMousePosition()
        {
            if (m_castMousePositions.TryGetValue(performingInstance, out var position))
                return position;
        ...
        /// <summary>
        /// Caches the mouse position for the current cast, so it remains unchanged during the cast even if the player moves the mouse.
        /// </summary>
        public virtual void CacheMousePosition()
        {
            var position = GetMousePosition();

            if (m_castMousePositions.ContainsKey(performingInstance))
                m_castMousePositions[performingInstance] = position;
            else
                m_castMousePositions.Add(performingInstance, position);
        }
        ...
        /// </summary>
        protected virtual bool ValidRequiredWeapon() => current.IsRequiredWeaponEquipped(m_entity);
