...
namespace PLAYERTWO.ARPGProject
{
    ...
    public class ItemAffixesEditor : Editor
    {
        ...
        string FilterFoldoutKey => $"ARPG.ItemAffixesEditor.filterFoldout.{target.GetEntityId()}";
        string SummaryFoldoutKey => $"ARPG.ItemAffixesEditor.summaryFoldout.{target.GetEntityId()}";
        string TierSummaryFoldoutKey =>
            $"ARPG.ItemAffixesEditor.tierSummaryFoldout.{target.GetEntityId()}";
#else
...
#endif
        ItemScope m_scopeFilter = ItemScope.None;
        ...
        List<int> GetFilteredIndices(List<ItemAffixes.AffixEntry> entries)
        {
            ...
            bool hasScopeFilter = m_scopeFilter != ItemScope.None;
        ...
        void DrawScopeSummary()
        {
            ...
            if (summaryFoldout)
            {
                ...
                foreach (
                    ItemScope scope in System.Enum.GetValues(
                        typeof(ItemScope)
                    )
                )
                {
        ...
        /// <summary>
        /// Returns true for single-bit scope values, excluding <see cref="ItemScope.None"/>
        /// and composite convenience flags like <see cref="ItemScope.Weapon"/> and
        /// <see cref="ItemScope.Armor"/>.
        /// </summary>
        static bool IsPrimitiveScope(ItemScope scope)
        {
            int value = (int)scope;
