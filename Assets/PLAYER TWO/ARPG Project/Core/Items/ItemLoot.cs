...
namespace PLAYERTWO.ARPGProject
{
    ...
    public class ItemLoot : MonoBehaviour
    {
        ...
        protected Coroutine m_lootRoutine;

        protected int m_lootIterationIndex;
        ...
        public virtual void Loot()
        {
            ...
            m_lootRoutine = StartCoroutine(LootRoutine());
        }

        protected virtual void OnDisable()
        {
            if (m_lootRoutine == null)
                return;
        ...
        /// <summary>
        /// Drops every remaining loot iteration immediately, skipping the
        /// per-drop delay. Used when the routine can't run on a coroutine
        /// (the GameObject is inactive) or gets interrupted mid-flight.
        /// </summary>
        protected virtual void DropRemainingLoot()
        {
            for (; m_lootIterationIndex <= stats.loopCount; m_lootIterationIndex++)
                PerformLootDrop();
        }

        /// <summary>
        /// Rolls and instantiates a single loot drop (item or money) at a random ground position.
        /// </summary>
        protected virtual void PerformLootDrop()
        {
