...
namespace PLAYERTWO.ARPGProject
{
    ...
    public partial class ItemInstance
    {
        ...
        /// <summary>
        /// Returns the display name of this Item Instance, incorporating its rarity name,
        /// assigned prefix/suffix, and socket summary.
        /// </summary>
        public virtual string GetDisplayName() => WithSocketSuffix(GetAffixedName());

        /// <summary>
        /// Returns the generated identity name, including its rarity naming convention and
        /// assigned prefix/suffix, without presentation details such as socket count.
        /// </summary>
        protected virtual string GetAffixedName()
        {
            if (rarityId < 0)
                return data.name;
            ...
            if (!db.itemRarities.IsIndexValid(rarityId))
                return data.name;

            var rarity = db.itemRarities[rarityId];
            var affixes = rarity.affixes;
            var rarityName = string.IsNullOrWhiteSpace(rarity.displayName)
                ? ""
                : $"{rarity.displayName} ";

            if (affixes == null)
                return $"{rarityName}{data.name}".Trim();
            ...
            return $"{rarityName}{prefix}{data.name}{suffix}".Trim();
        }

        /// <summary>
        /// Returns the generated item title used by the tooltip. Rarity and assigned affix
        /// names are part of the item's identity; socket count, Armor, Item Power, and other
        /// numeric properties are deliberately left to the tooltip detail sections.
        /// </summary>
