...
namespace PLAYERTWO.ARPGProject
{
    ...
    public class Level : Singleton<Level>
    {
        ...
        [Tooltip(
            "The name of the scene where the Town Portal leads to. Leave empty if the town is part of this same scene."
        )]
        public string townSceneName;

        [Tooltip(
            "The Town Portal representing where the player arrives when entering this scene through a Town Portal. Its Game Object should start disabled in the scene."
        )]
        public TownPortal townPortalExit;
        ...
        protected virtual void InitializePlayer()
        {
            if (Physics.Raycast(playerOrigin.position, Vector3.down, out var hit))
            {
                ...
                else
                {
                    ...
                    if (currentCharacter.usingTownPortal && townPortalExit)
                    {
                        townPortalExit.Activate();
                        var exitPoint = townPortalExit.GetSpacePoint();
                        player.Teleport(exitPoint.position, exitPoint.rotation);
                        currentCharacter.usingTownPortal = false;
                        return;
                    }
        ...
        )
        {
            ...
            money.playSpawnAnimation = !isRestoredFromSave;
            money.amount = amount;
        ...
        {
            if (townPortalExit)
                townPortalExit.SetAsReturnPortal();

            InitializePlayer();
            RestoreState();
        }

        protected virtual void InitializeTownPortalExit()
        {
            if (!townPortalExit)
                return;

            if (string.IsNullOrEmpty(currentCharacter.townPortalReturnScene))
                townPortalExit.Deactivate();
            else
                townPortalExit.Activate();
        }
