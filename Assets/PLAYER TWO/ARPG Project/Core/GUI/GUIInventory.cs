...
namespace PLAYERTWO.ARPGProject
{
    ...
    public class GUIInventory : MonoBehaviour, IPointerDownHandler, IDropHandler
    {
        ...
        /// <summary>
        /// Decrements the stack of a given Item Instance by one, fully removing it from the
        /// Inventory and destroying its GUI representation once the stack reaches zero. Used to
        /// finalize consumption of items consumed directly from the Inventory.
        /// </summary>
        /// <param name="item">The Item Instance you want to remove one unit from.</param>
        public virtual void RemoveStack(ItemInstance item)
        {
            if (item == null || item.stack <= 0 || !m_inventory.Contains(item))
                return;
            ...
            var guiItem = m_items.Find(i => i.item == item);

            if (guiItem && TryRemove(guiItem))
        ...
        protected virtual void UpdateMoney()
        {
            ...
            moneyText.text = m_inventory.money.ToMoneyString();
        }
