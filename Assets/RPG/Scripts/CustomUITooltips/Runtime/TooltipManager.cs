...
namespace CustomUITooltips
{
    ...
    public sealed class TooltipManager : MonoBehaviour
    {
        ...
        [Header("Defaults")]
        [Tooltip("Default visual and timing profile used when a tooltip does not provide an Override Profile.")]
        public TooltipProfile defaultProfile;

        [Tooltip("When enabled, this tooltip system persists across scene loads. Keep disabled for scene-specific tooltip overlays.")]
        ...
        private void BeginDelayedShow()
        {
            ...
            bool useUnscaledTime = currentProfile.useUnscaledTime;
            showRoutine = StartCoroutine(ShowAfterDelay(delay, useUnscaledTime, requestVersion));
        }

        private IEnumerator ShowAfterDelay(float delay, bool useUnscaledTime, int requestVersion)
        {
            if (delay > 0f)
            {
                if (useUnscaledTime)
            ...
            if (!IsCurrentShowRequest(requestVersion) || !EnsureVisualTree())
            {
                ClearShowRoutine(requestVersion);
                yield break;
            }

            // UI Toolkit needs a layout pass after DisplayStyle.Flex before resolvedStyle has final dimensions.
            // Keep the card hidden for that pass so it never flashes at the previous tooltip position.
            card.style.visibility = Visibility.Hidden;
        ...
        private bool IsCurrentShowRequest(int requestVersion)
        {
            return showRequestVersion == requestVersion && currentContent != null && currentProfile != null;
        }

        private void ClearShowRoutine(int requestVersion)
        {
            if (showRequestVersion == requestVersion)
                showRoutine = null;
        }
        ...
        private void BuildVisualTree()
        {
            ...
            bodyLabel.style.alignSelf = Align.Stretch;
            ResetLabelBox(titleLabel);
            ResetLabelBox(bodyLabel);
        ...
        private static void ResetLabelBox(Label label)
        {
        ...
        {
            bool hasTitle = !string.IsNullOrWhiteSpace(content.title);
            bool hasBody = !string.IsNullOrWhiteSpace(content.body);
            bool hasIcon = content.icon != null;
            bool titleOnly = hasTitle && !hasBody && !hasIcon;

            card.style.width = StyleKeyword.Auto;
            card.style.minWidth = titleOnly ? 0f : profile.minWidth;
            ...
            bodyLabel.style.display = hasBody ? DisplayStyle.Flex : DisplayStyle.None;
            ...
            titleLabel.style.unityTextAlign = profile.titleTextAlignment;
            bodyLabel.style.unityTextAlign = profile.bodyTextAlignment;
            titleLabel.enableRichText = profile.enableRichText;
            bodyLabel.enableRichText = profile.enableRichText;
            bodyLabel.style.marginTop = hasTitle && hasBody ? 4f : 0f;
        }
