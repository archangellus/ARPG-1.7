...
8. **Complete (existing-window runtime integration); prefab wiring required** — the existing `GUIBlacksmith` now owns Repair and Salvage tabs. Its Salvage tab supports manual row calls, junk, rarity, broad eligible, clear, preview, confirmation, and close. It displays deterministic totals and returned socketables.
...
4. On the **existing Blacksmith window**, keep `GUIBlacksmith` and add two tab buttons plus two panel roots. Assign `repairTabButton`, `salvageTabButton`, `repairTabPanel` (the existing repair/socket controls), and `salvageTabPanel` (the new child panel). Do **not** create another `GUIWindow`.
...
- Add and serialize the Salvage tab children/references on the existing Blacksmith window prefab and assign salvage settings on the existing Blacksmith NPC. No separate salvage window/provider should be created. The prefab was not modified automatically because UI layout/reference choices require Unity Editor serialization.
